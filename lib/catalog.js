...
function formatB2COrderSection() {
  ...
  // Backward-compat: support both old `weight_options` and new `weight_options_default`.
  const weightList = b.weight_options_default || b.weight_options || [];
  const weights = weightList.map(w => {
    const note = w.note ? ` (${w.note})` : '';
    return `${w.size} (~${w.cups_approx}잔)${note}`;
  }).join(', ');

  const zacaffeOnly = b.weight_options_zacaffe_only || [];
  const zacaffeNote = zacaffeOnly.length
    ? ` (ZACAFFÈ 한정: ${zacaffeOnly.map(w => w.size).join(', ')})`
    : '';

  // roast_options now uses page_label (was name). Fall back gracefully.
  const roastLines = (b.roast_options || []).map(r => `  · ${r.page_label || r.name} — ${r.notes}`);

  return [
    '=== B2C 원두 주문 옵션 ===',
    `- 주문 방식: ${b.ordering_model}`,
    `- 중량: ${weights}${zacaffeNote}`,
    `- 분쇄도: ${b.grind_options.join(', ')}`,
    `- 로스팅 신선도 옵션 (페이지 라벨 그대로 사용):`,
    ...roastLines,
  ].join('\n');
}
