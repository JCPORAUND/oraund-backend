...
// Scopes we ask for (admin-side). Must match exactly what's registered on the
// Cafe24 Developer Center app permissions page; otherwise we get invalid_scope.
// Order data already contains member_id + product info — that's all we need
// for blend recommendation, so we keep this minimal.
const SCOPES = ['mall.read_order'].join(',');
...
async function getCustomerOrders(memberId, opts = {}) {
  ...
  // Cafe24 requires date range and caps at 6 months. Default: last 180 days.
  const today = new Date();
  const startBack = new Date(today);
  startBack.setDate(today.getDate() - (opts.daysBack || 180));
  const fmt = (d) => d.toISOString().slice(0, 10);
  const startDate = opts.startDate || fmt(startBack);
  const endDate   = opts.endDate   || fmt(today);
